// PS - display only the even numbers from the linked list:
...
{
    printf("Even elements of the linked list are:\n");
    while(Head != NULL)
    {
        if((Head->data) % 2 == 0)
        {
...
int main()
{
    ...
    displayEven(First); // EVEN FUNCTION CALL

    int evenSum = SumEven(First);  // even-sum function
