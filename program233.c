// PS - index of the first occurrence of a number
...
int SearchFirstOccurrence(PNODE Head, int number)
{
    int position = 1;
    ...
    while (Head != NULL)
    {
        if (Head->data == number)
        {
            return position; // found, returning current index
        }
        Head = Head->next;
        position++;
    }
...
int main()
{
    ...
    int num = 10;
    int pos = SearchFirstOccurrence(First, num);

    if (pos != -1)
        printf("First occurrence of number %d is at position: %d\n", num, pos);
    else
